...
def apply_filter_custom():
    ...
    if entries:
        ...
        print("Núcleo de Convolução Personalizado: ")
        print(np.array(custom_filter))

        kernel_label.config(text="Núcleo de Convolução: \n" + str(np.array(custom_filter)))
...
def create_custom_filter_window():
    ...
    if custom_filter_window is not None:
...
# Cria um botão para o usuário aplicar um filtro personalizado
